...
Operators and job vertices in Flink have a name and description. 
While the name and description both explain what an operator or a job vertex is doing, they are used differently.

The name of the operator and job vertex will be used in the web UI, thread name, logging, metrics, etc.
The name of a job vertex is constructed based on the names of operators in it.
The name needs to be as concise as possible to avoid high pressure on external systems.

The description will be used in the execution plan and is displayed as the details of a job vertex in web UI.
The description of a job vertex is constructed based on the descriptions of operators in it.
...
By default, the description of a job vertex is formatted as a tree. 
Users can set `pipeline.vertex-description-mode` to `CASCADING`, if they want the description to be in the cascading format as in former versions.
...
Users can set `table.exec.simplify-operator-name-enabled` to `false`, if they want the name to include a detailed description as in former versions.

When the topology of the pipeline is complex, users can add a topological index in the name of vertex by setting `pipeline.vertex-name-include-index-prefix` to `true`.
This makes it easier find the vertex in the graph according to logs or metrics tags.
